...
public class ChangeStatusOfCoffeeSiteAsyncTask extends AsyncTask<Void, Void, Void> {
    ...
    public ChangeStatusOfCoffeeSiteAsyncTask(SITE_STATUS_ASYNC_REST_OPERATION status, CoffeeSite coffeeSite, LoggedInUser currentUser, CoffeeSiteService callingService) {
        ...
        tag = "SiteStAsynTask_" + this.requestedStatus.toString();
    }
    ...
    protected Void doInBackground(Void... voids) {
        ...
        if (currentUser != null) {
            ...
            switch (this.requestedStatus) {
                case ACTIVATE:
                    call = api.activateCoffeeSite(coffeeSite.getId());
                    break;
                case DEACTIVATE:
                    call = api.deactivateCoffeeSite(coffeeSite.getId());
                    break;
                case CANCEL:
                    call = api.cancelCoffeeSite(coffeeSite.getId());
                    break;
            }
                ...
                public void onResponse(Call<Boolean> call, Response<Boolean> response) {
                    if (response.isSuccessful()) {
                        if (response.body() != null) {
                            ...
                            callingService.sendCoffeeSiteStatusChangeResultToClient(requestedStatus, operationResult, "");
                        } else {
                            ...
                            callingService.sendCoffeeSiteStatusChangeResultToClient(requestedStatus,"", operationError);
                        }
                    } else {
                        try {
